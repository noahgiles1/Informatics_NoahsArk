...
public class MainController implements Initializable {
    ...
    = FXCollections.observableArrayList(
            ...
            new Disease("Influenza", "HoldingPage.fxml")
    );
	
	FilteredList<Disease> filteredData = new FilteredList<Disease>(data, p -> true); // adds the organised list to a filtered list so it can be sorted
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		{
			filteredData.setPredicate(p -> p.getName().toLowerCase().contains(searchField.getText().toLowerCase().trim()));
		});
	}
}
